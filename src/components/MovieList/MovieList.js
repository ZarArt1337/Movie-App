...
const fetchCast = () => {
  fetch(
    `https://api.themoviedb.org/3/movie/${385687}?api_key=${API_KEY}&append_to_response=credits`
  )
    .then(response => response.json())
    .then(data => {
      console.log("OBSADAAA:", data.credits.cast);
    });
};
...
const fetchActorInfo = () => {
  fetch(`https://api.themoviedb.org/3/person/64?api_key=${API_KEY}`)
    .then(response => response.json())
    .then(data => console.log("co to za aktor", data));
};
